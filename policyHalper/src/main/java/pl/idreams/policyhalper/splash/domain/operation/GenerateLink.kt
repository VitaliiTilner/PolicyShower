    ...
    operator fun invoke(
       ...
       val linkBuilder = StringBuilder()
        ...
        linkBuilder.apply {
            ...
            append("${crypto.decrypt("jcvltm")}=${policyValues.bundle}&")
            append("${crypto.decrypt("xcap")}=${parseToolsData.sL?.getOrNull(1) ?: "null"}&")
            append("${crypto.decrypt("lmd")}_${crypto.decrypt("smg")}=${policyValues.AFK}&")
            append("${crypto.decrypt("nj")}_${crypto.decrypt("ixx")}_" +
                    "${crypto.decrypt("ql")}=${policyValues.FID}&")
            append("${crypto.decrypt("nj")}_${crypto.decrypt("ib")}=${policyValues.FToken}&")
        }
